...
public class PlayerSMGAnimation : PlayerAnimation
{
    //모드 변경
    bool buff;
    ...
    {// 0 기본사격, 1 대각선, 2 위, 3 조준
        ...
        sk.AnimationState.AddEmptyAnimation(3, 0.05f, 0.1f);
    ...
    public override void StopAction(int kind)
    {
        ...
        {
            sk.AnimationState.SetEmptyAnimation(7, 0.1f);
            if (!buff)
            {
                sk.AnimationState.SetAnimation(7, "buff_on", false);
            }
            else
            {
                sk.AnimationState.SetAnimation(7, "buff_off", false);
            }
            sk.AnimationState.AddEmptyAnimation(7, 0.1f, 1f);


        }
        else if (kind == 3) // 스킨 변경을 위한
        {
            if (!buff)
            {
                sk.Skeleton.SetSkin("buff");
                sk.Skeleton.SetSlotsToSetupPose();
                buff = true;
            }
            else
