...
public class PlayerSMGAttack : PlayerAttack
{
    ...
    public override void stopAction()
    {
        if (!stopActionOnOff)
        {
            ...
            if (Input.GetKeyDown(KeyCode.LeftControl))
            {
                ...
                StartCoroutine(stopControllActionTime());
            }
        }
    ...
    IEnumerator stopControllActionTime() // smg만의 컨트롤 키 액션을 위한 코루틴
    {

        stopActionOnOff = true;
        PlInputAnimation.StopAction(stopActionKind);
        yield return new WaitForSeconds(1f);
        PlInputAnimation.StopAction(3);
        PlInputAnimation.StopActionDone();
